...
#include "ITTRLogger.generated.h"
 ...
 */
struct FTTRMessageLogInitializationOptions
{
	FTTRMessageLogInitializationOptions() {}
...
UENUM()
enum class ETTRLoggerLogLevel : uint8
{
	NoLogging = 0,
 ...
 * - on screen
 */
class TUTORIALSYSTEM_API ITTRLogger
{
	typedef ITTRLogger Self;
protected:
	ITTRLogger() {}
...
public:
	...
	// Create a new logger
	static ITTRLogger New() { return Self{}; }
	static ITTRLogger& Get()
	{
		static ITTRLogger Instance;
		return Instance;
	}
	...
	Self& DisableRedirectMessageLogLevels() { return SetRedirectMessageLogLevelsHigherThan(ETTRLoggerLogLevel::NoLogging); }
	Self& SetRedirectMessageLogLevelsHigherThan(ETTRLoggerLogLevel AfterOrEqualLevel)
	{
	...
	Self& SetOpenMessageLogLevelsHigherThan(ETTRLoggerLogLevel AfterOrEqualLevel)
	{
	...
	static void MessageLogRegisterLogName(FName LogName, const FText& LogLabel, const FTTRMessageLogInitializationOptions& InitOptions = {});
	...
	void Logf(ETTRLoggerLogLevel Level, const FmtType& Fmt, Types... Args)
	{
	...
	template <typename FmtType, typename... Types>
	void Errorf(const FmtType& Fmt, Types... Args) { Logf(ETTRLoggerLogLevel::Error, Fmt, Args...); }

	template <typename FmtType, typename... Types>
	void Warningf(const FmtType& Fmt, Types... Args) { Logf(ETTRLoggerLogLevel::Warning, Fmt, Args...); }

	template <typename FmtType, typename... Types>
	void Infof(const FmtType& Fmt, Types... Args) { Logf(ETTRLoggerLogLevel::Info, Fmt, Args...); }

	template <typename FmtType, typename... Types>
	void Debugf(const FmtType& Fmt, Types... Args) { Logf(ETTRLoggerLogLevel::Debug, Fmt, Args...); }

	template <typename FmtType, typename... Types>
	void Tracef(const FmtType& Fmt, Types... Args) { Logf(ETTRLoggerLogLevel::Trace, Fmt, Args...); }


	// void Fatal(const ANSICHAR* File, int32 Line, const FString& Message);
	void Log(ETTRLoggerLogLevel Level, const FString& Message);

	// TODO implement
	// void Fatal(const FString& Message) { Log(ENYLoggerLogLevel::Fatal, Message); }
	FORCEINLINE void Error(const FString& Message) { Log(ETTRLoggerLogLevel::Error, Message); }
	FORCEINLINE void Warning(const FString& Message) { Log(ETTRLoggerLogLevel::Warning, Message); }
	FORCEINLINE void Info(const FString& Message) { Log(ETTRLoggerLogLevel::Info, Message); }
	FORCEINLINE void Debug(const FString& Message) { Log(ETTRLoggerLogLevel::Debug, Message); }
	FORCEINLINE void Trace(const FString& Message) { Log(ETTRLoggerLogLevel::Trace, Message); }

protected:
	void VARARGS LogfImplementation(ETTRLoggerLogLevel Level, const TCHAR* Fmt, ...);

#if WITH_UNREAL_DEVELOPER_TOOLS
	static FMessageLogModule* GetMessageLogModule();
#endif // WITH_UNREAL_DEVELOPER_TOOLS

	virtual void LogScreen(ETTRLoggerLogLevel Level, const FString& Message);
	virtual void LogOutputLog(ETTRLoggerLogLevel Level, const FString& Message);
	virtual void LogMessageLog(ETTRLoggerLogLevel Level, const FString& Message);
	virtual void LogClientConsole(ETTRLoggerLogLevel Level, const FString& Message);

	static ELogVerbosity::Type GetUnrealLogTypeForLogLevel(ETTRLoggerLogLevel Level)
	{
	 	switch (Level)
		{
		...
		case ETTRLoggerLogLevel::Error:
			return ELogVerbosity::Error;

		case ETTRLoggerLogLevel::Warning:
			return ELogVerbosity::Warning;

		case ETTRLoggerLogLevel::Debug:
			return ELogVerbosity::Verbose;

		case ETTRLoggerLogLevel::Trace:
	...
	static EMessageSeverity::Type GetMessageSeverityForLogLevel(ETTRLoggerLogLevel Level)
	{
		switch (Level)
		{
		// case ENYLoggerLogLevel::Fatal:
	...
	FColor GetColorForLogLevel(ETTRLoggerLogLevel Level) const
	{
		switch (Level)
		{
		// case ENYLoggerLogLevel::Fatal:
	...
	static FOutputDevice* GetOutputDeviceFromLogLevel(ETTRLoggerLogLevel Level);
...
protected:
	...
	ETTRLoggerLogLevel RedirectMessageLogLevelsHigherThan = ETTRLoggerLogLevel::Warning;
	...
	ETTRLoggerLogLevel OpenMessageLogLevelsHigherThan = ETTRLoggerLogLevel::NoLogging;
