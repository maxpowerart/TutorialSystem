#include "TutorialSystem/Public/Logger/ITTRLogger.h"
...
#define TTR_GROWABLE_LOGF(SerializeFunc) \
...
ITTRLogger& ITTRLogger::SetClientConsolePlayerController(APlayerController* PC)
{
	PlayerController = PC;
	return *this;
}

FOutputDevice* ITTRLogger::GetOutputDeviceFromLogLevel(ETTRLoggerLogLevel Level)
{
#if NO_LOGGING
	return nullptr;
#endif
	...
	switch (Level)
	{
	case ETTRLoggerLogLevel::NoLogging:
		return nullptr;
...
#if WITH_UNREAL_DEVELOPER_TOOLS
FMessageLogModule* ITTRLogger::GetMessageLogModule()
{
	return FModuleManager::LoadModulePtr<FMessageLogModule>("MessageLog");
}
#endif // WITH_UNREAL_DEVELOPER_TOOLS

bool ITTRLogger::IsMessageLogNameRegistered(FName LogName)
{
#if WITH_UNREAL_DEVELOPER_TOOLS
...
void ITTRLogger::MessageLogRegisterLogName(FName LogName, const FText& LogLabel, const FTTRMessageLogInitializationOptions& InitOptions)
{
#if WITH_UNREAL_DEVELOPER_TOOLS
	FMessageLogModule* MessageLogModule = GetMessageLogModule();
	if (!MessageLogModule)
...
bool ITTRLogger::MessageLogUnregisterLogName(FName LogName)
{
#if WITH_UNREAL_DEVELOPER_TOOLS
	FMessageLogModule* MessageLogModule = GetMessageLogModule();
	if (!MessageLogModule)
...
#if WITH_UNREAL_DEVELOPER_TOOLS
TSharedPtr<IMessageLogListing> ITTRLogger::MessageLogGetLogNameListing(FName LogName)
{
	FMessageLogModule* MessageLogModule = GetMessageLogModule();
	if (!MessageLogModule)
	{
...
void ITTRLogger::MessageLogOpenLogName(FName LogName)
{
#if WITH_UNREAL_DEVELOPER_TOOLS
	FMessageLogModule* MessageLogModule = GetMessageLogModule();
	if (!MessageLogModule)
...
void ITTRLogger::LogfImplementation(ETTRLoggerLogLevel Level, const TCHAR* Fmt, ...)
{
#if !NO_LOGGING
	TTR_GROWABLE_LOGF(Log(Level, Buffer))
#endif // !NO_LOGGING
}

void ITTRLogger::Log(ETTRLoggerLogLevel Level, const FString& Message)
{
#if !NO_LOGGING
...
void ITTRLogger::LogScreen(ETTRLoggerLogLevel Level, const FString& Message)
{
	if (!GEngine)
	{
		return;
...
void ITTRLogger::LogClientConsole(ETTRLoggerLogLevel Level, const FString& Message)
{
...
void ITTRLogger::LogMessageLog(ETTRLoggerLogLevel Level, const FString& Message)
{
	// Should we be redirecting this message log because
	if (RedirectMessageLogLevelsHigherThan != ETTRLoggerLogLevel::NoLogging &&
...
void ITTRLogger::LogOutputLog(ETTRLoggerLogLevel Level, const FString& Message)
{
...
void ITTRLogger::ClearAllOnScreenLogs()
{
	if (!GEngine)
	{
		return;
